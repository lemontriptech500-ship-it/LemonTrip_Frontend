...
import { User, Mail, Phone, MapPin, Calendar, ShoppingBag, Settings, LogOut, Pencil, X, Check, Loader2 } from 'lucide-react'
...
export default function ProfileContent() {
  ...
  return (
    <div className="section-gap bg-[var(--color-background)]"><Container><div className="grid grid-cols-1 gap-6 lg:grid-cols-4">
      ...
      <div className="space-y-6 lg:col-span-3"><Card className="p-6"><div className="mb-5 flex items-center justify-between gap-4"><h2 className="text-h3">Personal Information</h2>{!isEditing ? <Button variant="outline" size="sm" icon={<Pencil size={15} />} onClick={startEditing}>Edit Profile</Button> : <div className="flex gap-2"><Button variant="ghost" size="sm" icon={<X size={15} />} onClick={cancelEditing} disabled={isSaving}>Cancel</Button><Button size="sm" icon={isSaving ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />} onClick={handleSave} disabled={isSaving}>{isSaving ? 'Saving...' : 'Save Changes'}</Button></div>}</div>{isEditing ? <div className="grid grid-cols-1 gap-4 md:grid-cols-2"><label className="text-sm font-medium text-[var(--color-text-secondary)]">Full Name<input value={name} onChange={(event) => setName(event.target.value)} className="mt-1 h-10 w-full rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 text-[var(--color-text-primary)]" /></label><div><Info icon={<Mail size={18} />} label="Email" value={user.email} /></div><label className="text-sm font-medium text-[var(--color-text-secondary)]">Phone<input value={phone} onChange={(event) => setPhone(event.target.value)} type="tel" placeholder="+91 98765 43210" className="mt-1 h-10 w-full rounded-[var(--radius-md)] border border-[var(--color-border)] bg-[var(--color-surface)] px-3 text-[var(--color-text-primary)]" /></label><Info icon={<MapPin size={18} />} label="Location" value="Not provided" />{error && <p className="text-sm font-medium text-[var(--color-error)] md:col-span-2" role="alert">{error}</p>}</div> : <div className="grid grid-cols-1 gap-5 md:grid-cols-2"><Info icon={<User size={18} />} label="Full Name" value={user.name} /><Info icon={<Mail size={18} />} label="Email" value={user.email} /><Info icon={<Phone size={18} />} label="Phone" value={user.phone || 'Not provided'} /><Info icon={<MapPin size={18} />} label="Location" value="Not provided" /></div>}</Card><Card className="p-6"><div className="mb-5 flex items-center justify-between"><h2 className="text-h3">Recent Bookings</h2><Button variant="ghost" size="sm" asChild><a href="/bookings">View All</a></Button></div><p className="text-sm text-[var(--color-text-muted)]">Your confirmed bookings will appear here after backend integration.</p></Card><Card className="p-6"><h2 className="text-h3 mb-5">Quick Actions</h2><div className="grid grid-cols-2 gap-3 md:grid-cols-4"><QuickAction icon={<Calendar size={20} />} label="Book Flight" /><QuickAction icon={<ShoppingBag size={20} />} label="My Trips" /><QuickAction icon={<Settings size={20} />} label="Settings" /><QuickAction icon={<User size={20} />} label="Support" /></div></Card></div>
    </div></Container></div>
  )
}
