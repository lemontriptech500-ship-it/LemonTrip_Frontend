...
export function RoomCard({ room, selections, onQuantityChange, selectedRoomCount, requestedRooms }: RoomCardProps) {
  ...
  return <article className="rounded-[var(--radius-lg)] border border-[var(--color-border)] bg-white p-5">
    ...
    <div className="mt-5 space-y-3" role="group" aria-label={`Rates for ${room.name}`}>
      {room.rates.map((rate) => {
        ...
        return <div key={rate.id} className={`rounded-[var(--radius-md)] border p-4 ${quantity ? 'border-[var(--color-primary)] bg-[var(--color-primary)]/5' : 'border-[var(--color-border-light)]'}`}>
