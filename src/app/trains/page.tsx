        ...
        <div className="mt-10 grid gap-5 lg:grid-cols-3">
          {mockTrains.map((train) => (
            <Card key={train.id} className="flex h-full flex-col" hover>
              <div className="flex items-center gap-3 border-b border-[var(--color-border-light)] pb-5"><div className="rounded-[var(--radius-md)] bg-[var(--color-primary)] p-3 text-[var(--green-dark)]"><TrainFront size={22} /></div><div><h2 className="font-bold text-[var(--color-text-primary)]">{train.name}</h2><p className="text-sm text-[var(--color-text-secondary)]">Train {train.number}</p></div></div>
              ...
              <div className="mt-auto space-y-4 border-t border-[var(--color-border-light)] pt-5"><div className="flex flex-wrap gap-2">{train.classes.map((item) => <span key={item} className="rounded-full bg-[var(--color-primary-soft)] px-2.5 py-1 text-xs font-semibold text-[var(--green-dark)]">{item}</span>)}</div><div className="flex items-center justify-between gap-3"><span className="font-semibold text-[var(--color-text-primary)]">{train.price}</span><Button variant="outline" size="sm" icon={<ArrowRight size={15} />} asChild><Link href={`/trains/${train.id}`}>View route</Link></Button></div></div>
            </Card>
          ))}
        </div>
